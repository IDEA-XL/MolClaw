...
- **Non-blocking Design** - Uses MolClaw scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. MolClaw already set up and running
...
Add Parallel AI MCP servers after the molclaw server:
```typescript
const mcpServers: Record<string, any> = {
  molclaw: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__molclaw__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__molclaw__send_message
   4. Use mcp__molclaw__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | container run -i --entrypoint /bin/echo molclaw-agent:latest "Container OK"
```
...
- Check task runs: `tail -f logs/molclaw.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.molclaw`
