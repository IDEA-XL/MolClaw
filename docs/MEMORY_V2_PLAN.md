# BioClaw Memory V2 Plan

## 1. 目的

本文件用于为 BioClaw 下一步的 memory 改造提供统一设计基线。

目标不是继续堆更多历史消息，而是把当前“按 group 复用 session”的机制，演进成一个真正分层、可观测、可治理的 memory 系统。

本计划同时参考了当前 BioClaw 实现，以及 `openclaw`、`claude-code`、`qwen-code` 在 memory、session、compaction、event streaming 方面的设计思路。

## 2. 当前 BioClaw Memory 现状

当前 BioClaw 的“memory”主要由 4 部分组成：
...
OpenClaw 对 BioClaw 的启发最大，因为 BioClaw 当前本质上就是一个更轻量的 OpenClaw/NanoClaw 路线。
...
这一步是 BioClaw 从“会话切断”升级为“会话收束”的关键。
...
BioClaw 当前最大的问题，不是“记得不够多”，而是“缺少 memory 分层与边界”。
...
做到这里，BioClaw 的 memory 设计就会从“能续聊”升级成“能管理上下文”。
